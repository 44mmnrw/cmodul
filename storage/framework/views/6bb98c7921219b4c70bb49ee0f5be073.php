...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="config-edit-right">
                <!-- Components Card (for both configurations Type 1 and components Type 2) -->
                <div class="card config-card">
                    <div class="config-card-header">
                        <svg class="card-icon" viewBox="0 0 20 20" fill="currentColor">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Берём максимальный индекс из существующих компонентов
        let maxIndex = -1;
        document.querySelectorAll('[data-component-id]').forEach(item => {
            const inputs = item.querySelectorAll('input[name^="components["]');
            inputs.forEach(input => {
                const match = input.name.match(/components\[(\d+)\]/);
                if (match) {
    ...
    function updateComponentOptions() {
        ...
        // Type 1 (конфигурация) может содержать компоненты Type 2
        // Type 2 (компонент) может содержать компоненты Type 3
        ...
        if (!requiredComponentType) {
            ...
            console.log('Cannot determine required component type');
            componentSelect.innerHTML = '<option value="" disabled>Ошибка определения типа компонентов</option>';
            return;
        }
    ...
    function addComponent() {
        ...
        item.innerHTML = `
            ...
            <button type="button" class="btn-remove-component" onclick="removeComponent(this); return false;">
                <svg viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"/>
                </svg>
            </button>
    ...
    // Функция удаления компонента с проверкой на пустой список
    function removeComponent(button) {
        const list = document.getElementById('components-list');
        button.closest('.component-edit-item').remove();
        
        // Если нет больше компонентов, добавляем empty-state
        if (list.querySelectorAll('.component-edit-item').length === 0) {
            list.innerHTML = '<div class="empty-state"><p>Компоненты не добавлены</p></div>';
        }
    }
</script>
