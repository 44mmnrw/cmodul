...
@section('title', 'История приходов')

@section('content')
<div class="receipts-wrapper">
    <!-- Заголовок страницы -->
    <div class="receipts-header">
        <a href="{{ $backUrl ?? '/receipts' }}" class="back-btn" title="Назад">
    ...
    <div class="table-container">
        ...
        <div class="table-wrapper">
            <table>
                ...
                <tbody>
                    @forelse($componentSummary ?? [] as $summary)
                        <tr>
                            <td>
                                <div class="cell-primary">{{ $summary['name'] ?? 'N/A' }}</div>
                                <div class="cell-secondary">{{ $summary['id'] ?? '' }}</div>
                            </td>
                            <td style="text-align: center;">
                                <span class="badge blue">{{ $summary['count'] ?? 0 }}</span>
                            </td>
                            <td style="text-align: right;">
                                <span class="cell-primary">{{ $summary['quantity'] ?? 0 }} шт</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
                ...
                <tbody>
                    @forelse($movements ?? [] as $movement)
                        <tr>
                            ...
                            <td>
                                <div class="cell-primary">{{ $movement->product->name ?? 'N/A' }}</div>
                                <div class="cell-secondary">{{ $movement->product->id ?? '' }}</div>
                            </td>
                            <td style="text-align: center;">
                                <span class="badge green">+{{ $movement->quantity }} шт</span>
                            </td>
                            <td>
                                <span style="color: #6b7280;">{{ $movement->reason ?? '—' }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    <!-- Пагинация -->
    @if($movements && method_exists($movements, 'hasPages') && $movements->hasPages())
