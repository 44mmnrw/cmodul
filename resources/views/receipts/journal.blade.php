    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            <div>
                ...
                <div class="stat-value">{{ $totalOperations ?? 0 }}</div>
            </div>
        ...
        <div class="stat-card">
            <div>
                ...
                <div class="stat-value">{{ $totalQuantity ?? 0 }}<small>шт</small></div>
            </div>
        ...
        <div class="stat-card">
            <div>
                ...
                <div class="stat-value">{{ $uniqueComponentsCount ?? 0 }}</div>
            </div>
            ...
            <table>
                ...
                <tbody>
                    @forelse($receipts ?? [] as $receipt)
                        <tr onclick="viewReceipt({{ $receipt['id'] ?? 0 }})" style="cursor: pointer;">
                            <td>
                                <span class="cell-primary">{{ $receipt['number'] ?? 'ПР-' . date('Y') . '-000' }}</span>
                            </td>
                            <td>
                                <div class="date-cell">
                                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <span>{{ $receipt['date'] ?? date('d.m.Y') }}</span>
                                </div>
                            </td>
                            <td style="text-align: center;">
                                <span class="badge blue">{{ $receipt['items_count'] ?? 0 }}</span>
                            </td>
                            <td style="text-align: center;">
                                <span class="badge green">+{{ $receipt['total_quantity'] ?? 0 }} шт</span>
                            </td>
                            <td>
                                <span style="color: #6b7280;">{{ $receipt['notes'] ?? '—' }}</span>
                            </td>
                            <td style="text-align: center;">
                                <button class="icon-btn" onclick="viewReceipt({{ $receipt['id'] ?? 0 }}); event.stopPropagation();" title="Просмотр">
    ...
    @if(count($receipts ?? []) >= 10)
        <div class="pagination-wrapper">
            <div style="text-align: center; color: #6b7280; padding: 20px;">
                Показано {{ count($receipts) }} из {{ $totalOperations }} документов
            </div>
        </div>
    @endif
</div>
