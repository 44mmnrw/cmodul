...
@section('title', 'Приходы компонентов')
...
<div class="receipts-wrapper">
    ...
    <!-- Форма -->
    <form action="{{ route('receipts.store') }}" method="POST" class="receipt-form">
        @csrf
                    ...
                    <table class="receipt-table">
                        ...
                        <tbody id="itemsTable">
                            <tr class="table-row" data-row-index="0">
                                    ...
                                    <select name="items[0][product_id]" class="form-input" required>
                                        ...
                                        @foreach($components as $component)
                                            <option value="{{ $component->id }}">
                                                {{ $component->name }}
                                                @if($component->stock)
                                                    (Осталось: {{ $component->stock->available ?? 0 }} шт)
                                                @endif
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <td>
                                    <input type="date" name="items[0][date]" class="form-input" value="{{ date('Y-m-d') }}" required>
                                </td>
                                <td>
